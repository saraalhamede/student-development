...
export default function StudentDashboard({ user, courses, materials, assignments, announcements, search, setSearch }) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-indigo-100 bg-gradient-to-r from-white via-indigo-50 to-sky-50 p-4 shadow-soft sm:p-6">
        <div className="grid items-center gap-6 lg:grid-cols-[1fr_260px]">
          <div className="min-w-0">
            ...
            <h1 className="mt-3 break-words text-2xl font-black text-slate-950 sm:text-3xl md:text-4xl">Welcome back, {user.name.split(" ")[0]}</h1>
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="min-w-0 space-y-6">
          <section className="card p-4 sm:p-5">
            <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><h2 className="text-xl font-black text-slate-950">My Courses</h2><span className="text-sm font-bold text-indigo-600">{shownCourses.length} shown</span></div>
            <div className="grid gap-5 sm:grid-cols-2 2xl:grid-cols-4">{shownCourses.slice(0, 4).map((course) => <CourseCard key={course.id} course={course} />)}</div>
